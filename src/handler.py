...
def classify_text(sequence, labels):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        "facebook/bart-large-mnli", local_files_only=False
    )  # Change this to False to download if not available locally
...
async def handler(job):
    ...
    return {"classification_result": classification_result, "device": str(device)}
